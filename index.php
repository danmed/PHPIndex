...
if (isset($plainTextPassword) && $passwordHash === '') {
    ...
    // For security, you might want to manually copy this hash into the script and remove the plain text password.
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_file'])) {
    if ($isLoggedIn && WRITE_PROTECTION_ENABLED) {
        ...
        if (!isset($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
            $feedbackMessage = 'CSRF token validation failed.';
            $feedbackType = 'error';
        } else {
            $filename = $_POST['filename'] ?? '';
            $content = $_POST['file_content'] ?? '';

            // Sanitize filename: remove directory traversal and slashes
            ...
            if (!empty($safeFilename)) {
                ...
                if (file_exists($newFilePath)) {
                    ...
                    $feedbackType = 'error';
                     ...
                     $feedbackMessage = 'Error: The directory is not writable.';
                     $feedbackType = 'error';
                } else {
        ...
        $feedbackMessage = 'Error: You must be logged in to create files.';
        $feedbackType = 'error';
    }
}
?>
...
<body class="bg-gray-100 dark:bg-gray-900 font-sans">
    ...
    <div class="container mx-auto p-4 sm:p-6 lg:p-8">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
            ...
            <!-- Controls: Filter and Create Button -->
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-2xl">
            ...
            <form method="POST" action="?path=<?= urlencode($relativePath) ?>">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="filename" id="filename" required placeholder="e.g., new_script.sh" class="mt-1 w-full p-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                </div>
                <div class="mb-4">
                    <label for="file_content" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Content</label>
                    <textarea name="file_content" id="file_content" rows="10" class="mt-1 w-full p-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 font-mono text-sm"></textarea>
                </div>
                <div class="flex justify-end gap-3">
